...
def _normalize_gap_record(record: Dict[str, Any]) -> Dict[str, Any]:
    limitations = record.get("limitations", [])
    if not isinstance(limitations, list):
        limitations = []

    theme = str(record.get("theme", "")).strip()
    research_gap = str(record.get("research_gap", "")).strip()
    potential_idea = str(record.get("potential_idea", "")).strip()

    if research_gap and (not theme or not potential_idea):
        parsed = parse_gap_response(research_gap)
        theme = theme or parsed["theme"]
        potential_idea = potential_idea or parsed["potential_idea"]
        research_gap = parsed["research_gap"]

    return {
        "cluster": int(record.get("cluster", 0)),
        "theme": theme or "Emerging Research Opportunity",
        "limitations": [item for item in limitations if isinstance(item, str) and item.strip()],
        "research_gap": research_gap or "Research gap details are unavailable.",
        "potential_idea": potential_idea or "Potential idea details are unavailable.",
    }
...
@st.cache_data
def load_paper_outputs() -> Dict[str, Dict[str, Any]]:
    """Load extracted paper JSON outputs from the outputs directory."""
    papers_data: Dict[str, Dict[str, Any]] = {}

    if not OUTPUTS_DIR.exists():
        return papers_data

    for file_path in sorted(OUTPUTS_DIR.glob("*.json")):
        if file_path.name == RESEARCH_GAPS_PATH.name:
            continue
        ...
        if isinstance(data, dict):
            papers_data[file_path.stem] = data
...
def build_faiss_index() -> Tuple[Any, np.ndarray | None, List | None]:
    ...
    try:
        ...
        index, embeddings, chunk_objects = create_vector_store(chunks)
        return index, embeddings, chunk_objects
    except Exception:
        return None, None, None
...
def perform_semantic_search(query: str, top_k: int = 5):
    """
    Perform semantic search using FAISS and generate RAG answer.
    ...
    try:
        ...
        retrieved_chunks = search_documents(query, index, chunks, k=top_k)
        ...
        try:
            ...
            answer = (
                ...
                f"Semantic search still retrieved {len(retrieved_chunks)} relevant document chunks."
            )
